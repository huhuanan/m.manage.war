...
var vui={
	util:{
		back:function(flag,msg){
			...
			if(this.openKey&&$.vue[this.openKey]){
				var vobject=$.vue[this.openKey];
				if(null!=this.rowIndex&&vobject.datas&&vobject.datas[this.rowIndex]){
					vobject.datas.sort();
					vobject.datas[this.rowIndex]['_expanded']=false;//没有实现收缩expand的功能
				}
				console.log(flag,msg);
				if(vobject.backHandler){
					vobject.backHandler(flag,msg);
				}
			}
		},
		fn_color:function(btn_type){
			if(btn_type=='primary'){
				return "#2d8cf0";
			}else if(btn_type=='info'){
				return "#2db7f5";
			}else if(btn_type=='success'){
				return "#19be6b";
			}else if(btn_type=='warning'){
				return "#ff9900";
			}else if(btn_type=='error'){
				return "#ed4014";
			}
		},
	...
	$.extend({
		...
		vueTableListMethods:{
			backHandler:function(success,msg){//打开窗体的回调
				...
				console.log(this.backEvent,this.openMode,this.backSuccess,msg);
				this.handlerResult(this.backEvent,this.openMode,this.backSuccess,msg);
			},
			handlerResult:function(backEvent,openMode,success,msg){
				console.log(backEvent,openMode,success,msg);
			...
			expandRender:function(h,params,key){
				console.log(params,key);
				var row={};
				for(var i=0;i<this.allColumns.length;i++){
					row[this.allColumns[i].field]=params.row[this.allColumns[i].key];
				}
				return h(this.$options.components['expand-row'], 
					{props: {
						'openKey':this.key,'rowIndex':params.index,'row':row,'raw':params.row,'allColumns':this.allColumns,'hiddenCols':this.hiddenCols
					}}
				);
			},
			...
			isHiddenCol:function(field){
				for (var i = 0; i < this.hiddenCols.length; i++) {
					if (this.hiddenCols[i] == field) return true;
				}
				return false;
			},
			query:function(){
				...
				if(this.columns){
					...
					for(var i=0;i<this.allColumns.length;i++){
						...
						if(this.hiddenCols.indexOf(col.field)==-1){
